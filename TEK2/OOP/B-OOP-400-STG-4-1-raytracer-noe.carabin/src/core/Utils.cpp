...
/**
 * @file Utils.cpp
 * @brief Implements utility functions for argument parsing, rendering, post-processing, and denoising in the raytracer application.
 * @author Noé Carabin
 * @version 1.0
 * @date 2025-05
 *
 * @details
 * Contient les fonctions utilitaires nécessaires au fonctionnement global du moteur de rendu :
 * - Parsing des arguments (threads, résolution, options)
 * - Construction des caméras et scènes
 * - Affichage dynamique de la barre de progression
 * - Rendu parallèle par pixel avec OpenMP
 * - Ajustements d’image (contraste, saturation)
 * - Export vers PPM et débruitage par filtre bilatéral.
 */
...
void Utils::PrintProgressBar(int percent,
                    ...
                    bool showProgress)
    {
        ...
        for (int j = 0; j < h; ++j) {
            Utils::Parallel::forLoop(0, w, [&](int i) {
...
void Utils::denoise(const std::vector<Math::Vector3D>& in,
