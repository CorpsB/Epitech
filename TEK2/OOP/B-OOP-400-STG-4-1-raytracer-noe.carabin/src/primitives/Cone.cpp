...
RayTracer::Cone::Cone(const Math::Point3D& a, const Math::Vector3D& ax, double angDeg, double h, const Math::Vector3D& col)
      : APrimitive(Material{"legacy", col, 0,0, {0,0,0}, 0,1}), apex(a), axis(ax / ax.length()),
        angle(angDeg * M_PI/180.0), height(h) {}
    
        RayTracer::Cone::Cone(const Math::Point3D& a, const Math::Vector3D& ax, double angDeg, double h, const Material& mat)
      : APrimitive(mat), apex(a), axis(ax / ax.length()), angle(angDeg * M_PI/180.0), height(h) {}
    
      RayTracer::AABB RayTracer::Cone::bounds() const {
    ...
    bool RayTracer::Cone::hits(const Ray& ray, double& tOut, HitRecord& rec) const
    {
