...
RayTracer::Cylinder::Cylinder(const Math::Point3D& b, const Math::Vector3D& ax, double r, double h, const Math::Vector3D& col)
    : APrimitive(Material{"legacy", col, 0.0f, 0.0f, Math::Vector3D{0,0,0}, 0.0f, 1.0f}), base(b), axis(ax / ax.length()),
        radius(r), height(h) {}

        RayTracer::Cylinder::Cylinder(const Math::Point3D& b, const Math::Vector3D& ax, double r, double h, const Material& mat)
    : APrimitive(mat), base(b), axis(ax / ax.length()), radius(r), height(h){}

    RayTracer::AABB RayTracer::Cylinder::bounds() const
    {
    ...
    bool RayTracer::Cylinder::hits(const Ray& ray, double& tOut, HitRecord& rec) const
    {
