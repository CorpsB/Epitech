...
RayTracer::Tetrahedron::Tetrahedron(const Math::Point3D& center, double size, const Material& mat) : APrimitive(mat)
    {
        generate(center, size, mat);
    }

    bool RayTracer::Tetrahedron::hits(const Ray& ray, double& tHit, HitRecord& rec) const
    {
        bool hit = false;
    ...
    RayTracer::AABB RayTracer::Tetrahedron::bounds() const
    {
        if (_faces.empty())
            return AABB{};
    ...
    void RayTracer::Tetrahedron::generate(const Math::Point3D& center, double size, const Material& mat)
    {
