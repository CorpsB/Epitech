...
void search_execut_env(pipe_t *pipe, int i, int size, char **table)
{
    if (pipe == NULL)
        return;
    if (redirection(pipe, i, size, table))
        return;
    if (i < size - 1) {
        close(pipe->fd[0]);
    ...
    if (i != 0) {
        close(pipe->old_fd[1]);
        dup2(pipe->old_fd[0], STDIN_FILENO);
        close(pipe->old_fd[0]);
    }
    return;
}

static int execve_command(char **command, char *str, char **env, pipe_t *pipe)
{
    int id = fork();
    int status;
    ...
    pipe_t *pipe)
{
    ...
    for (int i = 0; emplacement[i] != NULL; i++){
        ...
        if (access(cmd_path, R_OK | F_OK | X_OK) != -1)
            exit_code = execve_command(command, cmd_path, env, pipe);
            exit_code = bad_architecture(exit_code, command);
        }
    if (exit_code == -1)
...
static int run_command(char **command, infos_t *infos, pipe_t *pipe)
{
    bool is_exec = is_exec_func(command);
    int exit_code = 0;

    if (!is_exec)
        exit_code = exec_command(infos->env, command, infos->env_l, pipe);
    if (is_exec)
        exit_code = execve_command(command, command[0], infos->env, pipe);
    return exit_code;
}
...
int which_commands(char *str, infos_t *infos, pipe_t *pipe)
{
    char **command = NULL;
    int exit_code = -1;
    ...
    if (exit_code == -1)
        exit_code = run_command(command, infos, pipe);
    free_table(command);
    return exit_code;
}
...
int run_shell(infos_t *infos)
{
    ...
    clean_str = format_str(str, true);
    exit_code = execute_semilicon(clean_str, infos);
    free(clean_str);
    return exit_code;
}
