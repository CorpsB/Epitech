...
int sfeselector(int a, char *forme, int largeur, int precision)
{
    int i = 1;
    int j = 0;
    ...
    if (precision != 'NULL')
        ...
        cpt++)
            my_putchar('0');
    main_e(a, forme, largeur, precision);
...
int main_e(double nbr, char *forme, int largeur, int precision)
{
    if (precision == NULL) {
        precision = 6;
    }
...
int mini_e(va_list parametersInfos, char *forme, int largeur, int precision)
{
    double nbr = va_arg(parametersInfos, double);

    if (nbr >= 0 && (forme[1] == ' ' || forme[1] == '+')) {
        my_putchar(forme[1]);
    }
    if (nbr >= 0 && (forme[2] == ' ' || forme[2] == '+')) {
        my_putchar(forme[2]);
    }
    main_e(nbr, forme, largeur, precision);
}
